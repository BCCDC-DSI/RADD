...
if (length(args)>1) # Excecuted in an offline SLURM job
{
  ...
  if ( exists('inp_dir') == FALSE ) {
	  inp_dir = '/arc/project/st-ashapi01-1/bccs_mzml/2025/'  
  ...
  if ( exists('db_filename') == FALSE ) {  
	  ...
	  db_filename = '/arc/project/st-ashapi01-1/RADD_libraries/combined_db_20240708.csv' 
  }
  if ( exists( 'out_dir') == FALSE) { 
    out_dir = '/scratch/st-ashapi01-1/2025_new/' 
  }
...
first_b_line = which(db1$Compound.Name == "")[1]
db1 = db1[1:(first_b_line-1),]
db1$Retention.Time[ which(is.na(db1$Retention.Time)) ] = 0
...
print(  t(head(db1,1)) )
cat( '\n\nRunning copy under:\n\t/arc/project/st-ashapi01-1/git/ywtang/RADD/workflows/part1/go_loop_combined_20240920.R\n\n\n' )
...
{  
  ...
  else 
  {
    ...
    results = map(seq_along(compounds), ~ {
      ...
      rt_range = with(parent, c(Retention.Time*60 - Retention.Time.Window,
                                Retention.Time*60 + Retention.Time.Window))
      print(paste('rt_range', rt_range))
      ...
      ms2_match = map_lgl(seq_along(spectra), ~ {
        ...
        # Calculate total number of fragment m/z values and the number of matches
        total_fragments = length(fragments$Product.m.z)
        num_matches = sum(match$match)
        
        # Only keep if at least 50% of the fragments from the database match
        num_matches >= 0.5 * total_fragments
      })
