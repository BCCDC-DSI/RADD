...
go_all <- function(data_dir = "/arc/project/st-cfjell-1/ms_data/expedited_2023/mzML/",
                   output_dir = "/scratch/st-cfjell-1/output/PTC_data/",
...
parse_outputs <- function(xcms_data_dir="/scratch/st-cfjell-1/output/ms_data/expedited_2023",
                          output_file="/scratch/st-cfjell-1/output/ms_data/rts.csv") {
  rds_files = list.files(xcms_data_dir, pattern = "*.rds", full.names = TRUE)
  N = length(rds_files)
  lines = c()
  cnt = 0
  for(rds_file in rds_files){
    cnt = cnt + 1
    print(paste(cnt, 'of', N))
    sample = gsub('.rds', '', basename(rds_file))
    xcms_data = readRDS(rds_file)
    xcms_data_obj = xcms_data[[2]] # ms1_matches, a list
    found_compounds = c()
    rts = c()
    for(i in 1:length(xcms_data_obj)) {
      x = xcms_data_obj[[i]]
      if(length(x) > 0){
        compound = names(xcms_data_obj)[i]
        rt = x[['ms1']]$rt[1]
        lines = c(lines, paste(sample, compound, rt, sep=','))
      }
    }
  }
  conn = file(output_file)
...
if (length(args)==2) {
  print(paste0("Running go() with ", args[1], args[2]))
  go(args[1], args[2])
}
