...
compare_new_lib <- function(mzml_file,
  ...
  # db2_filename = paste(database_dir, "THERMO DB-NOV2022.csv", sep = i"/")

  print( paste('Input:', mzml_file) ) #  LT added captioning
  print( paste('Output:',output_dir ) ) # ditto
  print( paste('Database', database_file ) ) # ditto
  ...
  # add chromPeakSpectra results
  output = list(chromPeakSpectra = spectra_df, compounds=compounds )
  return( output ) # LT added return variables
}
...
if ( nargs==2 ) {
  print(paste0("Running compare_new_lib() with ", args[1], args[2]))
  go(args[1], args[2])
}
print( c( nargs, 'args given') )
